<%@page pageEncoding="UTF-8" isELIgnored="false" session="false"%>
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%>
<%@taglib prefix="f" uri="http://www.slim3.org/functions"%>
<%@ page import="org.slim3.controller.validator.Errors" %>
<%@ page import="com.etco.App" %>
<%@ page import="com.etco.model.*" %>
<%@ page import="com.etco.enums.*" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.HashMap" %>
<%@ page import="com.etco.Utils" %>
<%@ page import="java.text.SimpleDateFormat" %>
<%
String siteId = (String) request.getAttribute("siteId");
Errors errors =(Errors) request.getAttribute("errors");
String message =(String) request.getAttribute("message");

SitePage sitePage = (SitePage) request.getAttribute("sitePage");
boolean isEditMode = Boolean.valueOf((String) request.getAttribute("isEditMode"));

HashMap<String,String> pageTextResMap =(HashMap<String,String>) request.getAttribute("pageTextResMap");
HashMap<String,String> pageGcsResMap =(HashMap<String,String>) request.getAttribute("pageGcsResMap");
%>
<!DOCTYPE html>
<html lang="ja">
<head>
<jsp:include page="/template/T001/include-parts/html_head.jsp" />
<style>
a.image-change {
	position: absolute;
	top: 10px;
	left: 10px;
}
</style>
</head>
<body class="bg-pattern10 active">
<div class="outer-wrapper boxed">

	<!-- HEADER -->
	<jsp:include page="/template/T001/include-parts/main_header.jsp" />
	<!-- /HEADER -->

	<!-- PAGE HEADER -->
	<div class="page_header" style="position: relative;">
		<%if(isEditMode) { %>
		<a  class="btn btn-default image-change"
			href="/user/account/editGcsRes?objectType=page&parentKey=<%=sitePage.getKey().getName() %>&resId=<%=GcsResIds.T001_CONTACT_001.toString()%>">
			画像の変更
		</a>
		<%}%>
		<div class="page_header_parallax" style="background-image: url(<%=Utils.getResValue(pageGcsResMap, sitePage.getKey().getName(), GcsResIds.T001_CONTACT_001) %>">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
						<h3><%=sitePage.getName() %></h3>
					</div>
				</div>
			</div>
		</div>
		<div class="bcrumb-wrap">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
						<ul class="bcrumbs">
							<li><a href="index.html"><i class="fa fa-home"></i> Home</a></li>
							<li><%=sitePage.getName() %></li>
						</ul>
						<div class="clearfix"></div>
					</div>
				</div>
			</div>
		</div>
	</div>

	<!-- INNER CONTENT -->
	<div class="inner-content">
		<div class="container">
			<div class="row">
				<div class="col-md-8 margin30">
					<p>
						<%if(isEditMode) { %>
						<a data-toggle="modal" 
							data-backdrop="static"
							data-target="#textResModal" 
							style="color: #878787;"
							href="/user/account/ajax/editTextRes?editType=shortText&objectType=page&parentKey=<%=sitePage.getKey().getName() %>&resId=<%=TextResIds.T001_CONTACT_001.toString()%>">
							<span id="<%=Utils.getResKey(sitePage.getKey().getName(), TextResIds.T001_CONTACT_001)%>"><%=Utils.getResValue(pageTextResMap, sitePage.getKey().getName(), TextResIds.T001_CONTACT_001)%></span>
						</a>
						<%}else { %>
						<%=Utils.getResValue(pageTextResMap, sitePage.getKey().getName(), TextResIds.T001_CONTACT_001) %>
						<%} %>
					</p>
					<div class="form-contact">
						<form action="/site/contactEntry" method="post">
							<div class="row">
								<div class="col-md-6">
									<div class="row control-group">
										<div class="form-group col-xs-12 controls ${f:errorClass('name','has-error')}">
											<label>お名前<span>*</span></label>
											<%if (errors.containsKey("name")){ %>
												<label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i> ${errors.name}</label>
											<%} %>
											<input class="field-name form-control input-lg"  type="text" name="name" required="required">
											<p class="help-block"></p>
										</div>
									</div>
								</div>
								<div class="col-md-6">
									<div class="row control-group">
										<div class="form-group col-xs-12 controls ${f:errorClass('email','has-error')}">
											<label>メールアドレス<span>*</span></label>
											<%if (errors.containsKey("email")){ %>
												<label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i> ${errors.email}</label>
											<%} %>
											<input class="field-email form-control input-lg" type="text" name="email" required="required">
											<p class="help-block"></p>
										</div>
									</div>
								</div>
							</div>
							<div class="row">
								<div class="col-md-6">
									<div class="row control-group">
										<div class="form-group col-xs-12 controls ${f:errorClass('phoneNumber','has-error')}">
											<label>電話番号<span>*</span></label>
											<%if (errors.containsKey("phoneNumber")){ %>
												<label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i> ${errors.phoneNumber}</label>
											<%} %>
											<input class="field-phone form-control" type="tel" name="phoneNumber" required="required">
											<p class="help-block"></p>
										</div>
									</div>
								</div>
								<div class="col-md-6">
									<div class="row control-group">
										<div class="form-group col-xs-12 controls ${f:errorClass('companyName','has-error')}">
											<label>会社名</label>
											<%if (errors.containsKey("companyName")){ %>
												<label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i> ${errors.companyName}</label>
											<%} %>
											<input class="field-companyName form-control input-lg" type="text" name="companyName">
											<p class="help-block"></p>
										</div>
									</div>
								</div>
							</div>
							
							<div class="row control-group">
								<div class="form-group col-xs-12 controls ${f:errorClass('subject','has-error')}">
									<label>件名<span>*</span></label>
									<%if (errors.containsKey("subject")){ %>
										<label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i> ${errors.subject}</label>
									<%} %>
									<input type="text" class="form-control input-lg" name="subject" required="required">
									<p class="help-block"></p>
								</div>
							</div>
							<div class="row control-group">
								<div class="form-group col-xs-12 controls ${f:errorClass('message','has-error')}">
									<label>お問い合わせ内容<span>*</span></label>
									<%if (errors.containsKey("message")){ %>
										<label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i> ${errors.message}</label>
									<%} %>
									<textarea rows="5" class="field-comments form-control input-lg" name="message" required="required"></textarea>
									<p class="help-block"></p>
								</div>
							</div>
							<br>
							<div class="row">
								<div class="form-group col-xs-12">
									<input type="hidden" name="siteId" value="<%=siteId %>" />
									<button type="submit" class="btn-submit button btn-md">送信</button>
								</div>
							</div>
						</form>
					</div>
					<!--contact form-->
				</div>
				<div class="col-md-4">
					<h3 class="no-margin">
						<%if(isEditMode) { %>
						<a data-toggle="modal" 
							data-backdrop="static"
							data-target="#textResModal" 
							style="color: #000;"
							href="/user/account/ajax/editTextRes?editType=shortText&objectType=page&parentKey=<%=sitePage.getKey().getName() %>&resId=<%=TextResIds.T001_CONTACT_002.toString()%>">
							<span id="<%=Utils.getResKey(sitePage.getKey().getName(), TextResIds.T001_CONTACT_002)%>"><%=Utils.getResValue(pageTextResMap, sitePage.getKey().getName(), TextResIds.T001_CONTACT_002)%></span>
						</a>
						<%}else { %>
						<%=Utils.getResValue(pageTextResMap, sitePage.getKey().getName(), TextResIds.T001_CONTACT_002) %>
						<%} %>
					</h3>
					<div class="space20"></div>
					<ul class="contact-info">
						<li>
							<p><strong><i class="fa fa-map-marker"></i> Address:</strong> 東京都新宿区四谷4-3-3 FUKUYAビル2F-A</p>
						</li>
						<li>
							<p><strong><i class="fa fa-envelope"></i> Mail Us:</strong> <a href="#">info@kms3.com</a></p>
						</li>
						<li>
							<p><strong><i class="fa fa-phone"></i> Phone:</strong> 03-6380-6577</p>
						</li>
						<li>
							<p><strong><i class="fa fa-print"></i> Fax:</strong> 03-6380-6581</p>
						</li>
					</ul>
				</div>
			</div>
		</div>
	</div>

	<footer>
		<div class="container">
			<div class="row">
				<div class="col-md-9">
					<h4 class="space30">
						<%if(isEditMode) { %>
						<a data-toggle="modal" 
							data-backdrop="static"
							data-target="#textResModal" 
							style="color: #fff;"
							href="/user/account/ajax/editTextRes?editType=shortText&objectType=page&parentKey=<%=sitePage.getKey().getName() %>&resId=<%=TextResIds.T001_CONTACT_090.toString()%>">
							<span id="<%=Utils.getResKey(sitePage.getKey().getName(), TextResIds.T001_CONTACT_090)%>"><%=Utils.getResValue(pageTextResMap, sitePage.getKey().getName(), TextResIds.T001_CONTACT_090)%></span>
						</a>
						<%}else { %>
						<%=Utils.getResValue(pageTextResMap, sitePage.getKey().getName(), TextResIds.T001_CONTACT_090) %>
						<%} %>
					</h4>
			    	<!-- <img src="images/basic/logo-ico.png" class="img-responsive space20" width="70" alt=""/> -->
					<p>
						<%if(isEditMode) { %>
						<a data-toggle="modal" 
							data-backdrop="static"
							data-target="#textResModal" 
							style="color: #878787;"
							href="/user/account/ajax/editTextRes?editType=longText&objectType=page&parentKey=<%=sitePage.getKey().getName() %>&resId=<%=TextResIds.T001_CONTACT_091.toString()%>">
							<span id="<%=Utils.getResKey(sitePage.getKey().getName(), TextResIds.T001_CONTACT_091)%>"><%=Utils.getResValue(pageTextResMap, sitePage.getKey().getName(), TextResIds.T001_CONTACT_091)%></span>
						</a>
						<%}else { %>
						<%=Utils.getResValue(pageTextResMap, sitePage.getKey().getName(), TextResIds.T001_CONTACT_091) %>
						<%} %>
					</p>
                </div>
				<div class="col-md-3">
					<h4 class="space30">
						<%if(isEditMode) { %>
						<a data-toggle="modal" 
							data-backdrop="static"
							data-target="#textResModal" 
							style="color: #fff;"
							href="/user/account/ajax/editTextRes?editType=shortText&objectType=page&parentKey=<%=sitePage.getKey().getName() %>&resId=<%=TextResIds.T001_CONTACT_092.toString()%>">
							<span id="<%=Utils.getResKey(sitePage.getKey().getName(), TextResIds.T001_CONTACT_092)%>"><%=Utils.getResValue(pageTextResMap, sitePage.getKey().getName(), TextResIds.T001_CONTACT_092)%></span>
						</a>
						<%}else { %>
						<%=Utils.getResValue(pageTextResMap, sitePage.getKey().getName(), TextResIds.T001_CONTACT_092) %>
						<%} %>
					</h4>
			  		<ul class="c-info">
						<li><i class="fa fa-map-marker"></i> 〒160-0004<br>東京都新宿区四谷4-3-3 FUKUYAビル2F-A</li>
						<li><i class="fa fa-phone"></i> 03-6380-6577</li>
						<li><i class="fa fa-envelope-o"></i> info@kms3.com</li>
					</ul>
					<div class="clearfix space10"></div>
				</div>
			</div>
		</div>
	</footer>

	<!-- FOOTER -->
	<jsp:include page="/template/T001/include-parts/main_footer.jsp" />
	<!-- /FOOTER -->
</div>

<jsp:include page="/template/T001/include-parts/html_script.jsp" />
    
<%if(isEditMode) { %>
<jsp:include page="/user/account/ajax/dialog_modal.jsp">
	<jsp:param name="modelId" value="textResModal" />
</jsp:include>
<%} %>


</body>
</html>






